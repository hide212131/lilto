...
test("ProviderSettingsService は保存した設定を再読込できる", () => {
  ...
  const saveResult = service.save({
    ...
    toolExecution: {
      useWindowsIsolatedToolExecution: true
    }
  });
  ...
  assert.equal(state.toolExecution.useWindowsIsolatedToolExecution, true);
});
...
test("ProviderSettingsService は toolExecution が未設定でも既定値を補完する", () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "lilto-provider-"));
  const storagePath = path.join(tempDir, "providers.json");
  fs.writeFileSync(
    storagePath,
  ...
  assert.equal(state.toolExecution.useWindowsIsolatedToolExecution, false);
});
...
test("ProviderSettingsService migrates legacy sandbox flag", () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "lilto-provider-"));
  const storagePath = path.join(tempDir, "providers.json");
  fs.writeFileSync(
    storagePath,
