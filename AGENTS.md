...
* GUI を変更した場合は、まず `/live-ui-manual-verification` を実施し、障害解析や仕様確認ではこれを第一選択にする
* GUI 変更時の E2E はリグレッションテストとして扱い、すべての GUI 機能変更が完了した後、最終確認として最後に実施する
* Electron UI 自動化の基本方針は「主要なユーザー操作は Playwright、Electron 寄りの難しい箇所だけ WebdriverIO の Electron Service で補う」とする
* GUI 変更時の最終 E2E は上記方針に従って実施し、`npm run e2e:electron` を実行して成功終了と `test/artifacts/electron-e2e.png` の生成を確認する
