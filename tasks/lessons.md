...
| 変更内容 | ミス/課題 | 再発防止ルール |
|---|---|---|
| Windows タスクバー/アプリ用アイコンを白背景いっぱいに見せる修正で、最初は runtime 合成だけを直した。 | ユーザーの「小さい」は位置ずれではなく、元 PNG の透明余白と build/icon.png の単純コピーが原因だった。実行時アイコンだけ直しても packaged/taskbar 資産側の見た目が残る。 | アイコンサイズの不具合では、表示位置ではなく「画像内の透明余白」「build 資産」「runtime NativeImage 合成」を必ず分けて確認する。元画像は透明 bounds をトリムしてから背景へ合成し、build/icon.png も同じ処理で生成する。 |

## 2026-05-02

| 変更内容 | ミス/課題 | 再発防止ルール |
|---|---|---|
| GitHub Actions の x64 runner で MSVC 検出を直した後も、`build-native.js` が candidate を arm64 先行で評価したため、未導入の `aarch64-pc-windows-msvc` 向けに Rust compile を始めて落ちた。 | toolchain の存在確認と「今回欲しい target arch」は別問題で、複数 arch が見える環境で候補順を固定すると、存在するが不要な arch を先に掴んで CI だけ壊しやすい。 | Windows native build で複数 arch 候補を扱う場合は、runner や packaging target に合う arch を先に選ぶ。少なくとも x64 CI では x64 を優先し、arm64 は明示的に必要なときだけ fallback する。 |
| Windows x64 build が通るようになった後、`verify-dist-artifacts.js` の Windows helper 判定が `\Resources\...` 固定で、実際の `release\win-unpacked\resources\...` を拾えず偽失敗した。 | packaging 自体は成功していても、verify スクリプトの大小文字や OS 差分を雑に扱うと CI だけが落ちる。特に Windows は展開先の `resources` が lowercase でも、別 OS 用の `Resources` 条件を流用すると検証が壊れる。 | 配布物 verify では、OS ごとの実パス表記をログや生成物から確認してから判定を書く。Windows のパス比較は少なくとも `resources`/`Resources` の実際の出力差を吸収する形にし、packaging success 後の verify failure はまず判定ロジックの誤差を疑う。 |
...
| 変更内容 | ミス/課題 | 再発防止ルール |
|---|---|---|
| scheduler follow-up と通常会話が同時に動くとき、renderer の pending assistant と loop progress を単一グローバル state で持っていたため、後から選択された session や別 request の loop event に応答を書き戻してしまう不具合を修正した。 | 非同期応答の書き戻し先を「現在表示中の messages 配列の index」で持つと、session 切替や別 request の run_start が割り込んだ時点で対象が別会話へずれる。 | 複数 session/request が並行しうる chat UI では、pending message は index ではなく `conversationId + messageId` で追跡し、loop event も `requestId` と発火元 `conversationId` でフィルタしてから反映する。 |
...
| prebuild �� Electron �X�N���v�g��N�����鎞�� `ELECTRON_RUN_AS_NODE` ������p���A`require("electron").app` �� `undefined` �ɂȂ��� `app.whenReady()` �������� | `prepare-build-assets.js` �� `execFileSync(electronPath, ...)` �Őe���ϐ�����̂܂ܓn���Ă��� | Electron �{�̂�g���q�v���Z�X��N������O�� `env` �𕡐����� `ELECTRON_RUN_AS_NODE` ��폜���ēn���BElectron ���s�X�N���v�g�ł� `app` API ��g���O����� |

## 2026-05-03 taskbar icon correction

| Change | Mistake/Context | Rule to repeat |
|---|---|---|
| Fixed Windows icon sizing after repeatedly targeting tray/dock-like paths. | The user meant the Windows taskbar app icon. The relevant paths are BrowserWindow resolveWindowIcon and build/icon.png, not the tray icon. Rounded transparent corners also make the white background appear smaller in the taskbar. | For Windows taskbar icon bugs, inspect resolveWindowIcon and build icon assets first. Use a full-canvas opaque background when the user expects the background to fill the icon, and only touch tray code when the user explicitly says tray. |

## 2026-05-03 taskbar tray distinction addendum
