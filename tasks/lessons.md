...
| Windows で `npm run e2e:electrobun` を実行すると、E2E 開始前の Electrobun build が即失敗した。 | E2E スクリプトが `node_modules/.bin/electrobun.cmd` を `spawnSync` で直接実行しており、Windows では `EINVAL` になった。ビルド単体は通るため、根本原因はビルド処理ではなく `.cmd` の起動方法だった。 | Windows の CLI 呼び出しは必ず `resolveCliCommand` と `normalizeCommandArgs` を通す共通実装に寄せる。E2E や補助スクリプトで `spawnSync(<path-to-.cmd>)` を直書きしない。 |
