...
- [x] 1.1 設定スキーマに `useWindowsSandboxForTools`（boolean, デフォルト OFF）を追加する
- [x] 1.2 設定画面に Windows Sandbox 実行の ON/OFF トグルを追加し、変更を永続化できるようにする
- [x] 1.3 Windows 以外では当該トグルを非表示または無効化し、既存UIへの影響を最小化する

## 2. 実行モード分岐の実装

- [x] 2.1 Main 側にツール実行モード判定（`host` / `windows-sandbox`）を実装する
- [x] 2.2 Bash/Write 実行入口をモード判定経由に変更し、OFF 時は既存ホスト実行を維持する
- [x] 2.3 実行ログにモード・設定値・失敗ステップを出力し、トラブルシュート可能にする

## 3. Windows Sandbox 実行アダプタ

- [x] 3.1 `windows-sandbox-rs` を参考にした最小アダプタ（起動・投入・実行・結果回収）を追加する
