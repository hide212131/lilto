...
Lilt-AI は、PC 操作を代理実行する軽量エージェントを目標とし、Electron をアプリ基盤、Pi の `pi-coding-agent` をエージェント実行基盤として採用する方針である。現時点では要件が README レベルで、Main/Renderer の責務境界、IPC 契約、定期実行モデルが未定義なため、実装の着手点と検証観点が曖昧である。
