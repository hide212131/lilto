...
システムは、Settings モーダル内の設定メニューを `Providers & Models` として提供し、Claude と Custom Provider の設定導線を同一画面で提示しなければならない（MUST）。
...
システムは、Custom Provider として OpenAI Completions Compatible 接続先を登録・編集・保存できなければならない（MUST）。
